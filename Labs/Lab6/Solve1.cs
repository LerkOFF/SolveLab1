namespace SolveLabs.Labs.Lab6
{
    public struct Country
    {
        ...
        // Метод для сериализации страны в строку для текстового файла
    ...
    public static class Solve1
    {
        ...
        // Пути к файлам
        private const string LogFilePath = "LogFile_For_Lab6_Solve1.txt";
        private const string TableFilePath = "Table_for_Lab6_solve1.dat"; // Для бинарного файла
        private const string TextTableFilePath = "Table_for_Lab6_solve1.txt"; // Для текстового файла

        private static bool useBinary = true; // Так как у меня "ВАРИАНТ №2"
        ...
        private static void LoadTableFromFile()
        {
            if (useBinary)
            {
                ...
                {
                    using (var fs = new FileStream(TableFilePath, FileMode.Open))
                    using (var br = new BinaryReader(fs))
                    {
                        while (fs.Position < fs.Length)
                        {
                            ...
                            {
                                Name = br.ReadString(),
                                Capital = br.ReadString(),
                                Population = br.ReadInt64(),
                                GovernmentType = br.ReadString()
                            };
            ...
            {
                if (File.Exists(TextTableFilePath))
                {
                    var lines = File.ReadAllLines(TextTableFilePath);
        ...
        /// <summary>
        /// Сохраняет текущую таблицу в файл при выходе
        /// </summary>
        private static void SaveTableToFile()
        {
            if (useBinary)
            {
                using (var fs = new FileStream(TableFilePath, FileMode.Create))
                using (var bw = new BinaryWriter(fs))
                {
                    foreach (var country in countries)
                    {
                        ...
                        bw.Write(country.Population);
                        bw.Write(country.GovernmentType);
                    }
                }
            }
            else
            {
                ...
                File.WriteAllLines(TextTableFilePath, lines);
            }
        }
        ...
        private static void ViewTable()
        {
            ...
            // Сохранение таблицы после просмотра
            SaveTableToFile();
        }
        ...
        private static void AddEntry()
        {
            ...
            SaveTableToFile(); // Сохранение таблицы после добавления
        ...
        private static void DeleteEntry()
        {
            ...
            if (int.TryParse(Console.ReadLine(), out int index) && index >= 1 && index <= countries.Count)
            {
                ...
                SaveTableToFile(); // Сохранение таблицы после удаления
        ...
        private static void UpdateEntry()
        {
            ...
            if (int.TryParse(Console.ReadLine(), out int index) && index >= 1 && index <= countries.Count)
            {
                ...
                SaveTableToFile(); // Сохранение таблицы после обновления
        ...
        /// <summary>
        /// Отображает самый длительный период бездействия пользователя и сохраняет данные перед выходом
        /// </summary>
        private static void SaveAndExit()
        {
            SaveTableToFile(); // Сохранение таблицы при выходе
            Console.ForegroundColor = ConsoleColor.Yellow;
